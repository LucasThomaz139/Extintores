...
function adicionar($age) {
        ...
        try {
            ...
            $sql = "INSERT INTO controage(agendameto,servico,pagamento) VALUES(:agendamento,:servico,:pagamento)";
...
//           
     ...
     function verificador($age){
        ...
        try {
            $conecta = new PDO('mysql:host=127.0.0.1;dbname=extintores', 'root', '');
            $conecta->beginTransaction();
            $sql = "UPDATE servico SET  agendamento= :agendamento,servico= :servico, pagamento= :pagamento WHERE id =:id";
            //print_r($sql);
           $prepara = $conecta->prepare($sql);
